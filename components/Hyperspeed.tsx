...
interface EffectOptions {
  onSpeedUp?: () => void;
  onSlowDown?: () => void;
  distortion?: string | DistortionUniforms;
  length?: number;
  roadWidth?: number;
  islandWidth?: number;
  lanesPerRoad?: number;
...
const Hyperspeed = ({ effectOptions = defaultEffectOptions, className }: HyperspeedProps) => {
  ...
  useEffect(() => {
    ...
    const distortions: Record<string, DistortionUniforms> = {
      mountainDistortion: {
    ...
    class CarLights {
      ...
      constructor(webgl: any, opts: any, colors: number[], speed: [number, number], fade: THREE.Vector2) {
        this.webgl = webgl;
        this.options = opts;
        this.colors = colors;
        this.speed = speed;
      ...
      init() {
        ...
        let colors: THREE.Color[] = this.colors.map(c => new THREE.Color(c));
    ...
    class LightsSticks {
      ...
      constructor(webgl: any, opts: any) {
        this.webgl = webgl;
        this.options = opts;
      }
      ...
      init() {
        ...
        if (Array.isArray(opts.colors.sticks)) {
          colors = opts.colors.sticks.map((c: number) => new THREE.Color(c));
        } else {
          colors = [new THREE.Color(opts.colors.sticks)];
        }
    ...
    class App {
      ...
      constructor(containerEl: HTMLElement, opts: any) {
        this.options = opts;
    ...
    if (typeof resolvedOptions.distortion === 'string') {
      resolvedOptions.distortion = distortions[resolvedOptions.distortion] || distortions.turbulentDistortion;
    }
